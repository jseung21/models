import os
import subprocess
from asb_utils import download_model, remove_model_tar_file, model_input, model_dict, remake_config, check_time, set_log
import argparse
import shutil
import time
...
def user_input():
    ...
    config.add_argument('-l', '--label_file', help='Label file Location', default='./asb_label_map_with_label.pbtxt', type=str,required=False)
...
def transfer_learning(logger, model,args):
    ...
    if args['reset']:
        shutil.rmtree('./asb_train_dir/' + model_dict[model][0])
        os.mkdir('./asb_train_dir/' + model_dict[model][0])
        args['reset'] = False

    train_dir = './asb_train_dir/' + model_dict[model][0]
    config_file = './asb_model_conf/' + model_dict[model][1]
    try:
        subprocess.check_output(['python', './asb_train.py', ' --logtostderr', '--train_dir', train_dir,
                     '--pipeline_config_path', config_file])
    except:
        logger.error('Transfer learning Error')
        exit()
    end_time = time.time()
...
def export_model(logger, model, exam_num):
    ...
    if os.path.isdir('./asb_export_dir/' + model_dict[model][0]):
        shutil.rmtree('./asb_export_dir/' + model_dict[model][0])
    export_dir = './asb_export_dir/' + model_dict[model][0]
    config_file = './asb_model_conf/' + model_dict[model][1]
    trained_checkpoint = './asb_train_dir/' + model_dict[model][0] + '/model.ckpt-' + str(exam_num)
    try:
        subprocess.check_output(['python', 'export_inference_graph.py',
...
def evaluate_model(logger, model, num_steps):
    ...
    config_file = './asb_model_conf/' + model_dict[model][1]
    try:
        subprocess.check_output(['python', './asb_eval.py',
                     '--logtostderr',
                     '--pipeline_config_path', config_file,
                     '--checkpoint_dir', './asb_train_dir/' + model_dict[model][0],
                     '--eval_dir',  './asb_eval_dir/' + model_dict[model][0] ,
                     '--run_once','True'])
    except:
...
def main():
    ...
    if args['reset']:
        if os.path.isdir('./asb_eval_dir/' + model_dict[model][0]):
            shutil.rmtree('./asb_eval_dir/' + model_dict[model][0])
